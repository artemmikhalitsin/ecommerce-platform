...
var ProductDescriptionIdentityMap = require(rootPath + '/DataSource/IdentityMap/ProductDescriptionsIdentityMap.js');
...
var ProductDescriptionRepository = function () {
  /**
   * Creates the class' unit of work, identity map, tdg and the
   * the inventory item identity map
   */
  function ProductDescriptionRepository() {
    _classCallCheck(this, ProductDescriptionRepository);
  ...
  _createClass(ProductDescriptionRepository, [{
    key: 'getAll',
    ...
    value: function getByIds(ids) {
      ...
      var products = this.ProductDescriptionIM.get(ids);
    ...
    value: function save(products) {
      ...
      if (productIds.length > 0) {
        var context = this.getByIds(productIds);
        var allRecords = this.ProductDescriptionIM.getAll();
        for (var i = 0; i < products.length; i++) {
          if (context.findIndex(function (p) {
            return p.model_number == products[i].model_number;
          }) !== -1 && electronicsToUpdate.findIndex(function (e) {
            return e.model_number == products[i].model_number;
          }) === -1) {
            // Case: the product exists in our list of products
            // and hasn't already been processed
            electronicsToUpdate.push(products[i]);
          } else if (allRecords.findIndex(function (p) {
            return p.model_number == products[i].model_number;
