...
var ShoppingCart = function () {
  ...
  _createClass(ShoppingCart, [{
    ...
    value: function addToCart(item, modelNumber) {
      ...
      var _Object$keys$length = Object.keys(this.cartItems).length;

      var _checkPostcondition = function _checkPostcondition(it) {
        if (!(Object.keys(_this.cartItems).length === _Object$keys$length + 1)) {
          throw new Error('Function  postcondition failed: Object.keys(this.cartItems).length === old(Object.keys(this.cartItems).length) + 1');
        }
    ...
    value: function removeFromCart(item) {
      ...
      var _cartItems$length = this.cartItems.length;

      var _checkPostcondition2 = function _checkPostcondition2(it) {
        if (!!Object.keys(_this2.cartItems).includes(item)) {
          throw new Error('Function  postcondition failed: !Object.keys(this.cartItems).includes(item)');
        }

        if (!(_this2.cartItems.length === _cartItems$length - 1)) {
          throw new Error('Function  postcondition failed: this.cartItems.length === old(this.cartItems.length) - 1');
        }
