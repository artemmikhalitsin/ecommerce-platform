...
class InventoryItemRepository {
  ...
  save(items) {
    ...
    let itemIds = items.map((i) => i.model_number);

    if (itemIds.length > 0) {
    // Retrieve the items corresponding to given ids
    let context = this.getByIds(itemIds);
    // REVIEW: Consider removing old debug statements - Artem
    console.log('Context in repo is: ' + JSON.stringify(context));
    for (let i = 0; i < items.length; i++) {
      if (context.findIndex(
            (p) => p.model_number == items[i].model_number
            ) !== -1 &&
          electronicsToUpdate.findIndex(
            (e) => e.model_number == items[i].model_number
          ) === -1) {
            // Case: the item exists in our inventory
            // and hasn't already been processed
            electronicsToUpdate.push(items[i]);
      } else
      if (context.findIndex(
            (p) => p.model_number == items[i].model_number
            ) === -1 &&
    ...
    electronicsToDelete = context.filter(function(desc) {
      ...
      return electronicsToUpdate.findIndex(
        (e) => e.model_number == desc.model_number
      ) === -1;
    });
  }
